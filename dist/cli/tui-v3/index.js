...
import { createInitialAppState, appReducer, } from './state/app-state.js';
import { createInitialSessionState, sessionReducer, } from './state/session-state.js';
 ...
 */
export async function startTuiV3(workspaceRoot = process.cwd(), options = {}) {
    // --- State ---
    ...
    const rootComponent = {
        ...
        render(width) {
            ...
            if (sessionVisible) {
                ...
                const bordered = renderPaneWithBorder('Sess', sessContent, sessionWidth, contentHeight, appState.focusedPane === 'sessions');
            ...
            const msgBordered = renderPaneWithBorder('Messages', msgContent, messagesWidth, contentHeight, appState.focusedPane === 'messages');
        ...
        handleInput(key) {
            ...
            if (appState.focusedPane === 'context') {
                if (contextScroll.handleInput(key))
                    return true;
            }
            if (appState.focusedPane === 'sessions') {
                if (sessionsComp.handleInput(key))
                    return true;
            }
    ...
    function syncStateToComponents() {
