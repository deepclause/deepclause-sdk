    ...
    try {
        ...
        if (!hasProviderPrefix) {
            ...
            if (options.provider) {
                ...
                const match = results.find(r => r.providers.some(p => p.provider === options.provider || p.label === options.provider));
                if (!match) {
...
function hasRootSandbox(argv) {
    return argv.includes('--sandbox');
}
function readTuiVersion(argv) {
    ...
    if (eq)
        return eq.slice('--tui='.length);
    return 'v1';
}
function requiresInitializedWorkspace(argv) {
