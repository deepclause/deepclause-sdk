...
export const App = ({ workspaceRoot, sandbox }) => {
    const { exit } = useApp();
    ...
    return (_jsxs(Box, { flexDirection: "column", width: appState.columns, height: appState.rows, children: [_jsx(MenuBar, { sessionTitle: sessionTitle, busy: appState.busy }), _jsx(PaneLayout, { sessionPaneCollapsed: appState.sessionPaneCollapsed, focusedPane: appState.focusedPane, height: contentHeight, columns: appState.columns, children: {
                ...
                } }), _jsx(CommandInput, { active: appState.mode === 'command', busy: appState.busy, onSubmit: (text) => { void handleSubmit(text); }, onActivate: () => appDispatch({ type: 'SET_MODE', mode: 'command' }), onEscape: () => appDispatch({ type: 'SET_MODE', mode: 'normal' }) }), _jsx(StatusBar, { focusedPane: appState.focusedPane, autoScroll: appState.autoScroll, busy: appState.busy, mode: appState.mode })] }));
};
