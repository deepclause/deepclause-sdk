...
export function useSession({ workspaceRoot, dispatch, executionDispatch, sandbox }: UseSessionOptions): SessionActions {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    if (!activeSessionIdRef.current) {
      ...
      const sessions = await listConductorSessions(workspaceRoot);
      dispatch({ type: 'SET_SESSIONS', sessions });
      dispatch({ type: 'SET_ACTIVE_SESSION', id: session.id, detail: { ...session, messages: [] } as never });
    }
