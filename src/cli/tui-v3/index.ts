...
import {
  formatSkillCreatorSummary,
  parseCommandBarInput,
  parseSetModelCommandArgs,
  runFileCommand,
  runSkillCreatorCommand,
  runSlashCommand,
} from '../tui.js';
...
export async function startTuiV3(
  ...
  async function handleSubmit(text: string): Promise<void> {
      ...
      switch (parsed.name) {
        ...
        case 'run':
          ...
          await runDirectCommand(
            `/${parsed.name} ${parsed.rawArgs}`,
            (signal, onEvent) => runFileCommand(workspaceRoot, parsed.args[0], parsed.args.slice(1), {
        ...
        case 'skill-creator':
          ...
          await runDirectCommand(
            ...
            async (signal, onEvent) => {
              if (!sessionState.activeSessionId) await createNewSession();
              const result = await runSkillCreatorCommand(
                workspaceRoot,
    ...
    if (parsed.kind === 'skill') {
      await runDirectCommand(
        `/${parsed.name}${parsed.rawArgs ? ` ${parsed.rawArgs}` : ''}`,
        (signal, onEvent) => runSlashCommand(workspaceRoot, parsed.name, parsed.args, {
          sessionId: sessionState.activeSessionId ?? undefined,
          sandbox: options.sandbox,
          signal,
          onEvent,
...
function executionDmlLabel(logEvent: ConductorLogEvent): string {
  if (logEvent.scope === 'main') return 'conductor.dml';
  const slug = logEvent.childSlug ?? 'child';
  return slug.endsWith('.dml') ? slug : `${slug}.dml`;
}

export function formatToolArgs(args: Record<string, unknown> | undefined, maxLength = 100): string {
